...
(defn input->emitter-output
  ...
  and :emitter."
  [states]
  (mapv (fn [x] {:input (:io.pedestal.async-app/input x)
                :emitter (set (:io.pedestal.async-app/emitter-deltas x))}) states))
...
(defn standardize-results [results]
  (let [tree (atom tree/new-app-model)]
    (mapv (fn [r]
            (let [d (:io.pedestal.async-app/emitter-deltas r)
                  old-tree @tree
                  new-tree (swap! tree tree/apply-deltas d)]
              (assoc r :io.pedestal.async-app/emitter-deltas (tree/since-t new-tree (tree/t old-tree)))))
          results)))
...
(deftest test-simplest-possible-app
  (let [app (build simplest-possible-app)
    ...
    (is (= (first (drop 2 results))
           {:io.pedestal.async-app/input {msg/topic :model-a
                                          msg/type msg/init
                                          :value 0}
            :io.pedestal.async-app/subscriptions [[]]
            :data-model {:model-a 0}
            :io.pedestal.async-app/emitter-deltas [[:node-create [] :map]
                                                   [:node-create [:model-a] :map]
                                                   [:value [:model-a] nil 0]]}))
    (is (= (last results)
           {:io.pedestal.async-app/input {msg/topic :model-a :n 42}
            :data-model {:model-a 42}
            :io.pedestal.async-app/subscriptions [[]]
            :io.pedestal.async-app/emitter-deltas [[:value [:model-a] 0 42]]}))
      ...
      (let [output-state (atom [])
        ...
        (is (= @output-state
               [[[:x] 0]
                ...
                [[:z] 9999]
                [[:x] 12]]))
        (is (= (set (apply concat (map :io.pedestal.async-app/emitter-deltas results)))
               #{[:node-create [] :map]
                 [:node-create [:sum] :map]
                 [:value [:sum] nil 0.0]
                 [:node-create [:x] :map]
...
(deftest test-filter-deltas
  (is (= (filter-deltas {:io.pedestal.async-app/subscriptions [[:a :b] [:a :j]]}
                        [[:_ [:a]]
                         [:_ [:a :b]]
                         [:_ [:a :b :c]]
                         [:_ [:a :x :c :b]]
