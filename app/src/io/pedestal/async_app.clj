...
(ns ^:shared io.pedestal.async-app
    (:require [clojure.core.async.impl.channels :as channels]
              ...
              [io.pedestal.app.async-dataflow :as async-dataflow]
...
(defn- transact-one [cin flow]
  (let [process-message-in (chan)
        cout (process-message process-message-in flow)]
    (go (while true
          (let [[state message] (<! cin)]
            (>! process-message-in [(-> state
                                        (assoc ::input message)
